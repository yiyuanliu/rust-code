...
fn jump(mut key: u64, nbucket: u32) -> u32 {
    ...
    while j < nbucket as _ {
        b = j;
        key = (Wrapping(2862933555777941757_u64) * Wrapping(key) + Wrapping(1)).0;
...
fn parse_arg() -> (u32, u32, u32) {
    ...
    let arg: Vec<u32> = std::env::args()
        .skip(1)
        .map(|arg| arg.parse().expect(&arg))
        .collect();
...
fn test_dist(nbucket: u32, nkey: u32, nstripe: u32, distinct: bool) {
    ...
    println!(
        "  {}: min {:.6}%, max {:.6}%, avg {:.6}%, iops {:.4}k",
        if distinct { "distinct" } else { "random n" },
        min_percent,
        max_percent,
        1_f64 / nbucket as f64,
        nkey as f64 / 1000_f64 / duration.as_micros() as f64 * 1000000_f64
    );
}
...
fn test_moved(nbucket: u32, nkey: u32, nstripe: u32, distinct: bool, add_nbucket: u32) {
    ...
    println!(
        "  {}: total {}, moved {}, percent {:.2}%",
        if distinct { "distinct" } else { "random n" },
        nkey * nstripe,
        moved,
        moved as f64 / (nkey * nstripe) as f64 * 100.0
    );
}
